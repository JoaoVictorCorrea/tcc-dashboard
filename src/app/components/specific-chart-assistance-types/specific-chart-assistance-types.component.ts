...
export class SpecificChartAssistanceTypesComponent implements OnChanges, OnInit, OnDestroy {
  ...
  createChart() {
    ...
    const chartElement = document.getElementById('SpecificChartAssistanceTypes');

    if (chartElement) {

      // Chamar o método para verificar os dados e controlar a visibilidade
      this.checkChartDataVisibility(chartElement, qtdAtendimentoSocial, qtdAtendimentoRecepcao, qtdAtendimentoCadastramentoCadUnico, qtdAtendimentoAtualizacaoCadUnico, qtdAtendimentoVisitaDomiciliar);
  ...
  checkChartDataVisibility(chartElement: HTMLElement, qtdAtendimentoSocial: number[],
    qtdAtendimentoRecepcao: number[], qtdAtendimentoCadastramentoCadUnico: number[],
    qtdAtendimentoAtualizacaoCadUnico: number[], qtdAtendimentoVisitaDomiciliar: number[]): void {
    
    // Verificar se o elemento do gráfico existe e se todos os valores são iguais a zero
    if (chartElement && 
      qtdAtendimentoSocial.every(value => value === 0) && 
      qtdAtendimentoRecepcao.every(value => value === 0) && 
      qtdAtendimentoCadastramentoCadUnico.every(value => value === 0) && 
      qtdAtendimentoAtualizacaoCadUnico.every(value => value === 0) && 
      qtdAtendimentoVisitaDomiciliar.every(value => value === 0)) {
